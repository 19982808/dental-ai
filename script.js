...
async function askAI(userMsg, imageBase64=null) {
  ...
  try {
    ...
    // Try to read body whether ok or not
    let data;
    ...
    if (!res.ok) {
      ...
      if (res.status === 401) return "⚠️ API key rejected. Go to Vercel → Settings → Environment Variables and check that GROQ_API_KEY is set correctly, then redeploy.";
      if (res.status === 403) return "⚠️ Access denied. Your Groq API key may be invalid or expired — create a new one at console.groq.com.";
      if (res.status === 404) return "⚠️ The /api/chat endpoint wasn't found. Make sure api/chat.js is in your repo and Vercel has deployed it.";
      if (res.status === 429) return "I'm getting too many requests right now — give me a few seconds and try again.";
      if (res.status === 500) return `⚠️ Server error: ${errMsg || "Check Vercel function logs for details."}`;
      return `⚠️ Error ${res.status}: ${errMsg || "Something went wrong. Check Vercel logs."}`;
    }
    ...
    if (!data?.choices?.[0]?.message?.content) {
      ...
      return "I got an unexpected response — try sending that again.";
    }
    ...
    showTyping(false);
    chatHistory.pop(); // clean up failed message
    console.error("Network error:", e.message);

    if (e.message.includes("fetch")) {
      return "⚠️ Network error — the /api/chat endpoint isn't reachable. Make sure you're on the Vercel URL (not opening the file locally).";
    }
    return "⚠️ Connection failed. Check your internet and try again.";
  }
}
...
function selectPlan(planId) {
  ...
  // Open Pesapal modal pre-filled with plan details
  if (typeof openPesapal === "function") {
    const label  = selectedPlan.title;
    const amount = selectedPlan.price.replace(/,/g, "");
    openPesapal(label, isNaN(amount) ? "" : amount);
  }
}

function processPayment() {
  // This is now handled by submitPesapal() in the modal
  // Keeping as fallback for the inline WhatsApp form if shown
  ...
  if (!selectedPlan) { alert("No plan selected."); return; }
  ...
  const pc = document.getElementById("paymentCards");
  if (pf) pf.classList.add("hidden");
  if (pc) { pc.style.display = "flex"; pc.style.flexDirection = "column"; }
